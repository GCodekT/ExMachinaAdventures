...
function GiveVehicle(num)
	...
	elseif num==11 then
		model="BelazShot"		
	elseif num==12 then
		model="MirotvorecShot_2"		
    elseif num==13 then
		model="Cruiser01"		
	elseif num==14 then
		model="Dozer01"
	elseif num==15 then
		model="MazForPlayer01"
	elseif num==16 then
		model="Hunter03"
    elseif num==17 then
		model="MazForPlayer02"
	elseif num==18 then
		model="MazForPlayer03"
	elseif num==19 then
		model="Scout01"
	elseif num==20 then
		model="Scout02"
...
function immortal (md)
	if testcheat()~=1 then return end
	local mode=1
	if md then mode=md end
	GetPlayerVehicle():setImmortalMode(mode)  
end
...
--éêìÜàÖ
	if testcheat()~=1 then return end
			...
			local biggun={"vector01","vulcan01","flag01","kpvt01","rainmetal01","elephant01","odin01","bumblebee01","omega01"}
			local giantgun={"cyclops01","octopus01","hurricane01","rocketLauncher","big_swingfire01","hammer01"}
